    ...
    <section id="media" className="media-banner">
      <div style={{ maxWidth: "var(--max-w)", margin: "0 auto" }}>
        <div ref={ref} className={`reveal${visible ? " visible" : ""}`}>
          <p className="section-label">Media &amp; Presence</p>
          <h2 className="section-title">Beyond the Written Word</h2>
...
function MediaStat({ number, label, index }: { number: string; label: string; index: number }) {
  ...
  return (
    ...
    >
      <div className="media-stat-number">{number}</div>
      <div className="media-stat-label">{label}</div>
    </div>
  );
}
