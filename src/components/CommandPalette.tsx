...
export type BookmarkItem = { id: string; title: string; type: "book" | "video" };
...
export function toggleBookmark(item: BookmarkItem) {
  ...
  const next = exists ? prev.filter(b => b.id !== item.id) : [item, ...prev].slice(0, 20);
...
const NAV_ITEMS: Omit<CmdItem, "action">[] = [
  ...
  { id: "reading",  icon: "📖", label: "Reading Room",  keywords: "books recommendations library" },
  { id: "topics",   icon: "🗂️", label: "Topics",        keywords: "clusters research subjects" },
  { id: "timeline", icon: "⏳", label: "Timeline",      keywords: "heritage history milestones" },
  { id: "map",      icon: "🗺️", label: "Map",           keywords: "gujarat places locations" },
  { id: "citations", icon: "📎", label: "Citations",    keywords: "apa mla reference cite" },
];
    ...
    <div className="cmd-overlay" onClick={e => e.target === e.currentTarget && close()}>
      <div className="cmd-panel" role="dialog" aria-label="Command palette" aria-modal="true">
        ...
        <div className="cmd-tabs">
          <button className={`cmd-tab${tab === "search" ? " active" : ""}`} onClick={() => setTab("search")}>
            🔍 Search
          </button>
          <button className={`cmd-tab${tab === "saved" ? " active" : ""}`} onClick={() => setTab("saved")}>
            🔖 Saved {bookmarks.length > 0 && <span className="cmd-tab-badge">{bookmarks.length}</span>}
          </button>
        </div>
              ...
              {!q && history.length > 0 && (
                <div className="cmd-group">
                  <div className="cmd-group-label cmd-group-label-row">
                    ...
                    <button className="cmd-clear-history" onClick={() => { clearHistory(); setHistory([]); }}>Clear</button>
                  </div>
                  {history.map((h, i) => (
                    <button
                      key={h}
                    ...
                    >
                      <span className="cmd-item-icon" style={{ fontSize: "0.8rem" }}>🕐</span>
                      <span className="cmd-item-label">{h}</span>
                      <span className="cmd-item-arrow" style={{ fontSize: "0.7rem", opacity: 0.5 }}>search</span>
                    </button>
                  ))}
                </div>
              )}
        ...
        {tab === "saved" && (
          <div className="cmd-results">
            {bookmarkItems.length === 0 ? (
              <div className="cmd-empty" style={{ padding: "2.5rem 1rem", textAlign: "center" }}>
                <div style={{ fontSize: "2.5rem", marginBottom: "0.75rem" }}>🔖</div>
                <div style={{ fontWeight: 600, marginBottom: "0.25rem" }}>No saved items yet</div>
                <div style={{ fontSize: "0.78rem", opacity: 0.6 }}>Bookmark books and videos to find them here instantly.</div>
              </div>
            ) : (
              <div className="cmd-group">
                <div className="cmd-group-label">Saved Items ({bookmarks.length})</div>
                {bookmarkItems.map((item, i) => (
                  <button
                    ...
                    onMouseEnter={() => setSelectedIdx(i)}
                  >
                    <span className="cmd-item-icon" style={{ fontSize: "0.75rem", color: "var(--c-terracotta)" }}>{item.icon}</span>
                    <span className="cmd-item-label">{item.label}</span>
                    {item.sublabel && <span className="cmd-item-sub">{item.sublabel}</span>}
                    <span className="cmd-item-arrow">↵</span>
                  </button>
